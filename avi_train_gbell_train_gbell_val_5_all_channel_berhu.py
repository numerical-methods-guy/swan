...
def compute_stats_for_gbells_allfields_ic(
    ...
    # --- ref stds from ORIGINAL random IC (same idea as your wrapper) ---
    with torch.no_grad():
...
class SWELightningModule(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # latitude vector in degrees (1D [nlat]) for ParadisLoss
        # NOTE: solver.lats is 1D in radians in your setup
        lat_deg_1d = torch.rad2deg(torch.tensor(self.config["_lat_rad_1d"], device=self.device)).to(torch.float64)
        #lat_deg_1d = torch.tensor(self.config["_lat_deg_1d"], device=self.device, dtype=torch.float32)
        num_features = 3
        num_surface_vars = 3
        pressure_levels = torch.empty(0, dtype=torch.float32)  # SWE: no pressure levels
        output_name_order = ["phi", "u", "v"]

        self.loss_fn = ParadisLoss(
            loss_function="reversed_huber",
            lat_grid=lat_deg_1d,                         # 1D [nlat] degrees
            pressure_levels=pressure_levels,             # empty
            ...
            #delta_loss=1.0,                              # <-- set your delta here
            #apply_latitude_weights=False,                # <-- set True if you want area-weighted loss
            delta_loss=float(self.config.get("_delta_loss", 1.0)),
            apply_latitude_weights=bool(self.config.get("_apply_latitude_weights", False)),
        )
        print("lat[0:5] =", lat[:5].detach().cpu().tolist())
        print("step stats:", float((lat[1:]-lat[:-1]).min()), float((lat[1:]-lat[:-1]).max()))
...
class GaussianBellsAllFieldsWrapperWithWinds(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # scale matching: match per-channel stds from ORIGINAL random IC (in grid space)
        with torch.inference_mode():
            ref_spec = self.solver.random_initial_condition(mach=self.mach)
            ref_grid = self.solver.spec2grid(ref_spec)  # (3,nlat,nlon)

            # channel 0: remove mean (constant g*havg-ish) to get fluctuation scale
            ref0 = ref_grid[0]
            self.ref_std0 = (ref0 - ref0.mean()).std().clamp_min(1e-12)

            # channel 1 and 2: use fluctuation scale; subtract mean for safety
            ref1 = ref_grid[1]
            ref2 = ref_grid[2]
    ...
    def __getitem__(self, idx):
        with torch.inference_mode():
            # Build gaussian-bells grid for all 3 channels
            phi_mean = self.solver.gravity * self.solver.havg

            phi_grid = self._sample_bells_grid(
                idx, ref_std=self.ref_std0, mean=phi_mean, offset_base=0
            )

            c1_mean = 0.0 if self.channel1_mean_zero else 0.0
            c2_mean = 0.0 if self.channel2_mean_zero else 0.0

            vort_grid = self._sample_bells_grid(
                idx, ref_std=self.ref_std1, mean=c1_mean, offset_base=1000
            )
            div_grid = self._sample_bells_grid(
                idx, ref_std=self.ref_std2, mean=c2_mean, offset_base=2000
            )
...
def main():
    ...
    # stash latitude (radians, 1D [nlat]) into config so SWELightningModule can build BerHu loss
    lat_rad_1d = train_dataset.solver.lats.detach().cpu()
    
    config["_lat_rad_1d"] = lat_rad_1d
    config["_lat_rad_1d"] = train_dataset.solver.lats.detach().cpu().tolist()
    # Use a clean, uniform, ascending latitude grid in degrees for ParadisLoss
    #nlat = config["data"]["nlat"]
    #lat_deg_1d = torch.linspace(-90.0, 90.0, nlat, dtype=torch.float64)
    #config["_lat_deg_1d"] = lat_deg_1d.tolist()
    config["_delta_loss"] = known_args.delta_loss
    config["_apply_latitude_weights"] = known_args.apply_latitude_weights
