...
def apply_ic_smoothing_if_enabled(dataset, config):
    """
    If config has data.initial_condition.mode == 'smooth_random',
    patch dataset.solver.random_initial_condition to return smoother ICs.
    """
    ic_cfg = config.get("data", {}).get("initial_condition", {})
    ic_mode = ic_cfg.get("mode", "random")

    if ic_mode != "smooth_random":
        print("[IC] Forecast: using default random initial condition (no smoothing).")
        return
    ...
    dataset.solver.random_initial_condition = _smooth_random_initial_condition_wrapper(
        dataset.solver,
        k_cut=k_cut,
        power=power,
    ...
    print(
        f"[IC] Forecast: smooth_random enabled: k_cut={k_cut}, power={power}, "
...
def main():
    ...
    # NEW: apply same IC smoothing as training, if enabled in YAML
    apply_ic_smoothing_if_enabled(dataset, config)
