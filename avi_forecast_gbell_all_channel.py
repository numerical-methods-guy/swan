...
def make_gaussian_bells_grid(
    ...
    if signed:
        signs = torch.where(
            _rand(device, dtype, (K,), seed=seed + 50) < 0.5,
            -torch.ones_like(amp),
            torch.ones_like(amp),
        )
        amp = amp * signs
...
def autoregressive_inference_with_winds(
    ...
    with torch.no_grad():
        ...
        if use_gbells:
            ...
            # reference stds from the random IC in grid space
            ref0 = ref_grid[0]
            ref1 = ref_grid[1]
            ref2 = ref_grid[2]
            ...
            # means: phi has physical mean g*havg, vort/div mean 0
            phi_mean = dataset.solver.gravity * dataset.solver.havg

            phi_grid  = make_gaussian_bells_grid(
                solver=dataset.solver,
                ref_std=ref_std0,
                seed=seed + 0,
                mean=phi_mean,
            ...
            vort_grid = make_gaussian_bells_grid(
                solver=dataset.solver,
                ref_std=ref_std1,
                seed=seed + 1000,
                mean=0.0,
            ...
            div_grid  = make_gaussian_bells_grid(
                solver=dataset.solver,
                ref_std=ref_std2,
                seed=seed + 2000,
                mean=0.0,
            ...
            # convert full 3-channel grid -> spec (replaces ALL channels)
            ic_grid = torch.stack([phi_grid, vort_grid, div_grid], dim=0)
            ic = dataset.solver.grid2spec(ic_grid)
            ic = torch.tril(ic)
