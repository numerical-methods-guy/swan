...
def autoregressive_inference_with_winds(
    ...
    with torch.no_grad():
        ...
        burn_in = int(burn_in)
        assert burn_in >= 0
        if solver_total_steps is None:
            solver_total_steps = burn_in + autoreg_steps
        else:
            solver_total_steps = int(solver_total_steps)
        assert solver_total_steps >= burn_in + autoreg_steps
        if use_gbells:
        ...
        #uspec = ic.clone()
        # --- burn-in: advance numerical solver first ---
        uspec = ic.clone()
        for _ in range(burn_in):
            uspec = dataset.solver.timestep(uspec, nsteps)

# now uspec is solver state at step = burn_in
        start_grid = dataset.solver.spec2grid(uspec)
        start_uv   = dataset.solver.getuv(uspec[1:])

# NN initial state = solver(burn_in)
        prd_fields = (start_grid - inp_mean) / torch.sqrt(inp_var)
        prd_fields = prd_fields.unsqueeze(0)

        prd_winds = (start_uv - wind_mean) / torch.sqrt(wind_var)
        prd_winds = prd_winds.unsqueeze(0)
        # Save "NN initial condition" (this corresponds to solver step = burn_in)
        prd_uv_grid = start_uv
        outputs = {"fields": prd_fields[0].cpu(), "winds": prd_uv_grid.cpu(),
           "solver_step": burn_in, "nn_step": 0}
        torch.save(outputs, os.path.join(output_dir, f"prediction_{0:0{pad_width}d}.pt"))

        truth_outputs = {"fields": prd_fields[0].cpu(), "winds": prd_uv_grid.cpu(),
                 "solver_step": burn_in, "nn_step": 0}
        torch.save(truth_outputs, os.path.join(output_dir, f"truth_{0:0{pad_width}d}.pt"))
...
def main():
    ...
    parser.add_argument("--burn_in", type=int, default=0,
                    help="Number of initial solver steps to skip before starting NN forecast.")
    parser.add_argument("--solver_total_steps", type=int, default=None,
                    help="If set, run solver to this final step. If not set, uses burn_in + autoreg_steps.")
    ...
    if use_winds:
        results = autoregressive_inference_with_winds(
            ...
            solver_total_steps=args.solver_total_steps,
        )
    else:
