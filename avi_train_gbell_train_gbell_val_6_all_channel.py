...
def _run_rollout_eval_case(pl_module, case_name, ic_spec, autoreg_steps=5):
    ...
    for key, vals in metrics_data.items():
        ...
        print(f"{case_name} {key:12s}: {vals_t.mean().item():.6f} ± {vals_t.std(unbiased=False).item():.6f}", flush=True)
...
def main():
    ...
    if config["training"]["pretrain_epochs"] > 0 and known_args.resume_from is None:
        ...
        trainer = pl.Trainer(
            ...
            logger=logger,
            callbacks=[checkpoint_callback, lr_monitor, WilliamsonRolloutCallback(autoreg_steps=5)],
    ...
    if config["training"]["finetune_epochs"] > 0:
        ...
        finetune_trainer = pl.Trainer(
            ...
            callbacks=[finetune_checkpoint, finetune_lr_monitor, WilliamsonRolloutCallback(autoreg_steps=5)],
            accelerator="gpu" if torch.cuda.is_available() else "cpu",
            devices=1,
            precision=precision,
            log_every_n_steps=config["training"]["log_every_n_steps"],
