...
class SWELightningModule(pl.LightningModule):
    ...
    def _debug_check_output(self, prd, batch_idx, rollout_step):
        import logging
        if torch.isnan(prd).any() or torch.isinf(prd).any():
            logging.warning(
                f"[DEBUG] NaN/Inf in model output: batch={batch_idx} rollout_step={rollout_step} "
                f"nan={torch.isnan(prd).any().item()} inf={torch.isinf(prd).any().item()}"
            )
        ...
        for name, param in self.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                    logging.warning(
                        f"[DEBUG] NaN/Inf in gradient: param={name} "
                        f"nan={torch.isnan(param.grad).any().item()} "
                        f"inf={torch.isinf(param.grad).any().item()}"
                    )
                    break
    ...
    def on_before_optimizer_step(self, optimizer):
        ...
        for name, param in self.named_parameters():
            if torch.isnan(param.data).any() or torch.isinf(param.data).any():
                logging.warning(
                    f"[DEBUG] NaN/Inf in weights before optimizer step: param={name}"
                )
                break
