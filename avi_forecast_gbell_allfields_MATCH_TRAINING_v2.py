...
def make_gaussian_bells_allfields_ic_spec_training_exact(
    solver,
    idx,
    seed,
    ref_field_mean,
    ...
    """
    Gaussian bells in ALL THREE channels (phi, vorticity, divergence),
    matching the rollout-loss training code logic:

      - same bell-count defaults
      - same sigma defaults
      - same signed default
      - same seeded offset style
      - each field channel scaled by 200-random-IC per-channel mean/std
    """
    device = solver.lap.device
    dtype  = solver.lap.dtype
    ...
    ref_mean0 = ref_field_mean[0, 0, 0]
    ref_mean1 = ref_field_mean[1, 0, 0]
    ref_mean2 = ref_field_mean[2, 0, 0]

    ref_std0 = ref_field_std[0, 0, 0].clamp_min(1e-12)
    ref_std1 = ref_field_std[1, 0, 0].clamp_min(1e-12)
    ref_std2 = ref_field_std[2, 0, 0].clamp_min(1e-12)
    ...
    def _sample_bells_grid(mean_c, std_c, offset_base):
        uK = _rand((1,), offset=offset_base + 5).item()
        K = int(k_min + math.floor(uK * (k_max - k_min + 1)))
        K = max(k_min, min(k_max, K))

        u = 2.0 * _rand((K,), offset=offset_base + 10) - 1.0
        lat0 = torch.asin(u)
        lon0 = 2.0 * math.pi * _rand((K,), offset=offset_base + 20)

        sigma = sigma_min + (sigma_max - sigma_min) * _rand((K,), offset=offset_base + 30)

        amp = _rand((K,), offset=offset_base + 40)
        if signed:
            signs = torch.where(
                _rand((K,), offset=offset_base + 50) < 0.5,
                -torch.ones_like(amp),
                torch.ones_like(amp),
            )
            amp = amp * signs
        ...
        bump = torch.zeros(solver.nlat, solver.nlon, device=device, dtype=dtype)
        for j in range(K):
            gamma = _great_circle_distance(lat, lon, lat0[j].view(1, 1), lon0[j].view(1, 1))
            bump = bump + amp[j] * torch.exp(-(gamma * gamma) / (2.0 * sigma[j] * sigma[j]))

        bump = bump - bump.mean()
        bump = bump / (bump.std() + 1e-6)

        return (mean_c + std_c * bump).to(dtype)

    phi_grid  = _sample_bells_grid(ref_mean0, ref_std0, offset_base=0)
    vort_grid = _sample_bells_grid(ref_mean1, ref_std1, offset_base=1000)
    div_grid  = _sample_bells_grid(ref_mean2, ref_std2, offset_base=2000)
...
def autoregressive_inference_with_winds(
    ...
    with torch.no_grad():
        ...
        for step in range(1, autoreg_steps + 1):
            ...
            prd_spec = dataset.solver.grid2spec(prd_unnorm.squeeze(0))
...
def main():
    ...
    parser.add_argument("--gbells_allfields", action="store_true",
                        help="Use Gaussian bells in all 3 channels, scaled like rollout-loss training.")
...
#)
    ...
    if int(args.gbells_allfields) + int(args.williamson_case2) + int(args.williamson_case6) > 1:
        raise ValueError("Choose at most one of --gbells_allfields, --williamson_case2, --williamson_case6")
    #gbells_cfg = dict(
    ...
    gbells_val_seed = base_seed + 12345
    ...
    # Need these before G-stats if all-fields gbells uses them
    ...
    if args.gbells_allfields:
        G_field_mean, G_field_var, G_wind_mean, G_wind_var = compute_stats_for_ic_distribution(
            solver=solver,
            make_ic_spec_fn=lambda i: make_gaussian_bells_allfields_ic_spec_training_exact(
                solver=solver,
                idx=i,
                seed=base_seed,
                ref_field_mean=R_field_mean,
                ref_field_std=R_field_std,
                k_min=gbells_cfg["k_min"],
                k_max=gbells_cfg["k_max"],
                sigma_min_deg=gbells_cfg["sigma_min_deg"],
                sigma_max_deg=gbells_cfg["sigma_max_deg"],
                signed=gbells_cfg["signed"],
            ),
            num_samples=ns,
            device=device,
        )
    ...
    if args.gbells_allfields:
        dataset.inp_mean = G_field_mean.to(device)
        dataset.inp_var  = torch.clamp(G_field_var.to(device), min=1e-8)

        if hasattr(dataset, "wind_mean"):
            dataset.wind_mean = G_wind_mean.to(device)
            dataset.wind_var  = torch.clamp(G_wind_var.to(device), min=1e-8)
    else:
    ...
    if args.gbells_allfields:
        ic_spec = make_gaussian_bells_allfields_ic_spec_training_exact(
            solver=dataset.solver,
            idx=0,
            seed=gbells_val_seed,
            ref_field_mean=R_field_mean,
            ref_field_std=R_field_std,
            k_min=gbells_cfg["k_min"],
            k_max=gbells_cfg["k_max"],
