...
def compute_energy_spectra(fields, sht):
    """
    Compute (isotropic) spectra on the sphere by binning by spherical-harmonic degree l
    and summing over m.

    Args:
        fields: Tensor (batch, 3, nlat, nlon) containing [h, vorticity, divergence]
        sht: RealSHT object for spherical harmonic transforms

    Returns:
        Dict with spectra arrays indexed by l (called "wavenumbers" here).
    """
    h = fields[:, 0:1]
    vort = fields[:, 1:2]
    div = fields[:, 2:3]
    ...
    # h_power etc are now (B, l, M) where l ~ max degree l+1, M ~ number of m-modes stored
    B, l, M = h_power.shape

    # Keep the same "max_k" style truncation you had (avoid pushing too high in l)
    nlat = fields.shape[-2]
    max_l = min(l, nlat // 2)

    h_power = h_power[:, :max_l, :]
    vort_power = vort_power[:, :max_l, :]
    div_power = div_power[:, :max_l, :]

    # Weighting over m:
    # If RealSHT stores only m >= 0 (common for real transforms), then m>0 should be doubled
    # to account for +/- m pairs.
    #
    # Heuristic: if M is much larger than l, it might be storing full m (including negatives),
    # in which case we should NOT double. For typical nlat=128,nlon=256, we expect M ~ 129 and l ~ 128.
    if M > (l + 2):
        # likely full m storage: no doubling
        m_weights = torch.ones(M, device=fields.device, dtype=h_power.dtype)
    else:
        # likely half-spectrum (m >= 0): double m>0
        m_weights = torch.ones(M, device=fields.device, dtype=h_power.dtype)
        if M > 1:
            m_weights[1:] = 2.0

    # Spectrum vs l: sum over m using weights
    rot_spectrum = (vort_power * m_weights.view(1, 1, -1)).sum(dim=-1)  # (B, max_l)
    div_spectrum = (div_power * m_weights.view(1, 1, -1)).sum(dim=-1)
    pot_spectrum = (h_power * m_weights.view(1, 1, -1)).sum(dim=-1)
    ...
    return {
        "rotational": rot_spectrum.detach().cpu().numpy(),
        "divergent": div_spectrum.detach().cpu().numpy(),
        "potential": pot_spectrum.detach().cpu().numpy(),
        "total": total_spectrum.detach().cpu().numpy(),
