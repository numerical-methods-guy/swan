...
class SWERolloutLightningModule(pl.LightningModule):
    ...
    def _bundle_on_device(self):
        ...
        out = {
            ...
            "wind_mean": bundle["wind_mean"].to(self.device),
            "wind_var": bundle["wind_var"].to(self.device),
    ...
    def _rollout_and_loss(self, inp_fields, inp_winds, tar_seq_fields, tar_seq_winds):
        """
        inp_fields: (B,3,H,W)
        inp_winds:  (B,2,H,W)
        tar_seq_fields: (B,K,3,H,W)
        tar_seq_winds:  (B,K,2,H,W)
        ...
        if self.burn_in > 0:
            prd = tar_seq_fields[:, self.burn_in - 1]
            cur_winds = tar_seq_winds[:, self.burn_in - 1]
    ...
    def training_step(self, batch, batch_idx):
        if self.use_winds:
            ...
            loss = self._rollout_and_loss(inp_fields, inp_winds, tar_seq_fields, tar_seq_winds)
        else:
