...
class ReversedHuberLossS2(torch.nn.Module):
    ...
    def __init__(
        ...
        nlon: int,
        grid: str,
        lat_grid_deg: torch.Tensor,   # <-- must be provided from solver.lats
        delta: float = 1.0,
        ...
        lat_w = self._compute_latitude_weights(lat_grid_deg)  # shape [H], unit-mean
    ...
    def _compute_latitude_weights(self, grid_lat_deg: torch.Tensor) -> torch.Tensor:
        """
        Copied from ParadisLoss._compute_latitude_weights (GraphCast-consistent, unit-mean).
        Expects a 1D uniform latitude vector in degrees.
        """
        ...
        has_poles = torch.isclose(lat_min, lat.new_tensor(-90.0), atol=1e-6) and torch.isclose(
            lat_max, lat.new_tensor(90.0), atol=1e-6
        )
        ...
        weights = weights / weights.mean()  # unit-mean normalization
        return weights.to(dtype=grid_lat_deg.dtype)

    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        pred/target: (B,C,H,W) or (C,H,W). Returns scalar.
        """
        if pred.dim() == 3:
        ...
        # piecewise reverse huber (continuous + C1 at d)
...
class SWELightningModule(pl.LightningModule):
    ...
    def __init__(self, config, loss_delta: float = 1.0, lat_grid_deg: torch.Tensor = None):
        super().__init__()
        ...
        if lat_grid_deg is None:
            raise ValueError("lat_grid_deg must be provided from create_datasets (solver.lats converted to degrees).")
...
def create_datasets(config, device):
    ...
    lat_grid_deg = (train_dataset.solver.lats * 180.0 / math.pi).to(torch.float32)
    return train_dataset, val_dataset, lat_grid_deg
...
def main():
    ...
    train_dataset, val_dataset, lat_grid_deg = create_datasets(config, device)
