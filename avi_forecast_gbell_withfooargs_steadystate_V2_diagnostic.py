...
def _isfinite(x):
    return torch.isfinite(x).all().item() if torch.is_tensor(x) else True

def _stat(name, x, max_print=6):
    if x is None:
        print(f"{name}: None")
        return
    ...
    with torch.no_grad():
        ...
        # guard empty
        if flat.numel() == 0:
            print(f"{name}: EMPTY tensor shape={tuple(x.shape)}")
            return
        q = torch.quantile(flat, torch.tensor([0.0, 0.01, 0.5, 0.99, 1.0], device=flat.device))
        print(
            f"{name}: shape={tuple(x.shape)} dtype={x.dtype} device={x.device} "
            f"finite={_isfinite(x)} "
            f"min={q[0].item():.6g} p01={q[1].item():.6g} "
            f"med={q[2].item():.6g} p99={q[3].item():.6g} max={q[4].item():.6g} "
            f"mean={x.mean().item():.6g} std={x.std().item():.6g}"
        )

def _stat_channels(prefix, x3):
    # x3: (3,nlat,nlon) or (B,3,nlat,nlon)
    if x3 is None or not torch.is_tensor(x3):
        _stat(prefix, x3)
        return
    if x3.dim() == 3:
        _stat(prefix, x3)
        for c, nm in enumerate(["ch0", "ch1", "ch2"]):
            _stat(f"{prefix}[{c}]({nm})", x3[c])
    elif x3.dim() == 4:
        _stat(prefix, x3)
        for c, nm in enumerate(["ch0", "ch1", "ch2"]):
            _stat(f"{prefix}[0,{c}]({nm})", x3[0, c])
    else:
        _stat(prefix, x3)

def _check_close(name, a, b, rtol=1e-5, atol=1e-8):
    if (a is None) or (b is None) or (not torch.is_tensor(a)) or (not torch.is_tensor(b)):
        print(f"{name}: cannot compare (None/non-tensor)")
        return
    with torch.no_grad():
        diff = (a - b).abs()
        rel = diff / (b.abs() + 1e-12)
        print(
            f"{name}: abs max={diff.max().item():.6g} mean={diff.mean().item():.6g} "
            f"rel max={rel.max().item():.6g} mean={rel.mean().item():.6g}"
        )
...
def autoregressive_inference_with_winds(
    ...
    with torch.no_grad():
        ...
        print("\n=== DIAG: right after normalization of IC ===")
        _stat_channels("raw grid(ic)", dataset.solver.spec2grid(ic))
        _stat("inp_mean", inp_mean)
        _stat("inp_var", inp_var)
        _stat_channels("prd_fields (supposedly normalized)", prd_fields)
        prd_fields = prd_fields.unsqueeze(0)
        _stat_channels("prd_fields (after unsqueeze)", prd_fields)
        ...
        if save_plots:
            ...
            print("\n=== DIAG: about to plot ===")
            _stat(f"plot tensor prd_fields[0,{plot_channel}]", prd_fields[0, plot_channel])
            pred_data = prd_fields[0, plot_channel].cpu().numpy()
            ax = fig.add_subplot(1, 1, 1)
            im = ax.imshow(pred_data, vmin=-250, vmax=-150, cmap="twilight_shifted")
        ...
        for step in range(1, autoreg_steps + 1):
            ...
            if save_plots:
                ...
                im1 = ax1.imshow(pred_data, vmin=-250, vmax=-150, cmap="twilight_shifted")
                ax1.set_title(f"Prediction (t={step})", fontsize=12, fontweight="bold")
                ax1.axis("off")

                ax2 = fig.add_subplot(1, 3, 2)
                im2 = ax2.imshow(truth_data, vmin=-250, vmax=-150, cmap="twilight_shifted")
                ax2.set_title(
...
#)
    ...
    print("\n=== DIAG: main() grid0 ===")
    _stat_channels("grid0 raw", grid0)

    if hasattr(dataset, "inp_mean"):
        grid0_norm = (grid0 - dataset.inp_mean.to(grid0.device)) / torch.sqrt(dataset.inp_var.to(grid0.device))
        _stat_channels("grid0_norm (using dataset stats)", grid0_norm)
