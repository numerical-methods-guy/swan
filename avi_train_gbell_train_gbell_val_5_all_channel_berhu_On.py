...
class ReversedHuberLossS2(nn.Module):
    """
    Reverse Huber / inverted Huber:
      - linear penalty for small errors |e| <= delta
      - quadratic penalty for large errors |e| > delta
    Continuous and C1 at delta via:
        rho(a)=a                           if a<=delta
        rho(a)=(a^2+delta^2)/(2*delta)     if a>delta
    Includes simple latitude area-weighting using cos(lat).
    """

    def __init__(self, nlat: int, nlon: int, grid: str, delta: float = 1.0, eps: float = 1e-12):
        super().__init__()
        self.nlat = nlat
        self.nlon = nlon
        self.grid = grid
        ...
        lat = torch.linspace(-0.5 * math.pi, 0.5 * math.pi, nlat)
        w = torch.cos(lat).clamp_min(0.0)          # (nlat,)
        w = w / (w.sum() + eps)                    # normalize
        self.register_buffer("lat_w", w.view(1, 1, nlat, 1), persistent=False)
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss_pt = torch.where(e <= d, e, (e * e + d * d) / (2.0 * d))

        # sum over channels, then area-weight
        loss_pt = loss_pt.sum(dim=1, keepdim=True)         # (B,1,H,W)
        loss_w = loss_pt * self.lat_w.to(loss_pt.dtype)    # weight by latitude
        return loss_w.mean(dim=-1).sum(dim=-2).mean()      # mean over lon, sum over lat, mean over batch
...
class SWELightningModule(pl.LightningModule):
    ...
    def __init__(self, config, loss_delta: float = 1.0):
        super().__init__()
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Delta for reversed huber loss: linear for |e|<=delta, quadratic for |e|>delta (continuous).",
    )
    ...
    model = SWELightningModule(config, loss_delta=known_args.loss_delta)
    precision = 32
