...
def plot_energy_spectra(pred_spectra, truth_spectra, step, output_path, model_name="Model"):
    ...
    titles = ["Rotational Kinetic Energy", "Divergent Kinetic Energy", "Potential Energy", "Total Energy"]
    ...
    for idx, (title, key) in enumerate(zip(titles, keys)):
        ...
        ax.loglog(k_ref, 1e4 * k_ref ** (-3.0), "k:", linewidth=1.5, alpha=0.5, label=r"$k^{-3}$")
        ax.loglog(k_ref, 1e3 * k_ref ** (-5.0 / 3.0), "k-.", linewidth=1.5, alpha=0.5, label=r"$k^{-5/3}$")
...
def _run_single_ic_inference(
    ...
    if output_dir is not None:
        ...
        torch.save(init_outputs, os.path.join(output_dir, f"{prefix}prediction_{0:0{pad_width}d}.pt"))
        torch.save(init_outputs, os.path.join(output_dir, f"{prefix}truth_{0:0{pad_width}d}.pt"))

        if save_plots:
            fig = plt.figure(figsize=(6, 5))
            ax = fig.add_subplot(1, 1, 1)
            im = ax.imshow(prd_fields[0, plot_channel].cpu().numpy(), vmin=-4, vmax=4, cmap="twilight_shifted")
            ax.set_title("Initial Condition (t=0)", fontsize=12, fontweight="bold")
            ax.axis("off")
            fig.subplots_adjust(bottom=0.15)
            fig.colorbar(im, cax=fig.add_axes([0.15, 0.05, 0.7, 0.03]), orientation="horizontal")
            plt.savefig(os.path.join(output_dir, f"{prefix}comparison_{0:0{pad_width}d}.png"),
                        dpi=150, bbox_inches="tight")
            plt.close()

        if spectral_analysis:
            init_spectra = compute_energy_spectra(prd_fields, dataset.sht)
            plot_energy_spectra(init_spectra, init_spectra, 0,
    ...
    for step in range(1, autoreg_steps + 1):
        ...
        if output_dir is not None:
            torch.save({"fields": prd_fields[0].cpu(), "winds": prd_uv_grid.cpu()},
                       os.path.join(output_dir, f"{prefix}prediction_{step:0{pad_width}d}.pt"))
            torch.save({"fields": ref_fields[0].cpu(), "winds": ref_uv_grid.cpu()},
            ...
            if save_plots:
                ...
                fig.colorbar(im1, cax=fig.add_axes([0.08, 0.08, 0.22, 0.03]), orientation="horizontal")
                fig.colorbar(im2, cax=fig.add_axes([0.39, 0.08, 0.22, 0.03]), orientation="horizontal")
                fig.colorbar(im3, cax=fig.add_axes([0.70, 0.08, 0.22, 0.03]), orientation="horizontal")
                plt.savefig(os.path.join(output_dir, f"{prefix}comparison_{step:0{pad_width}d}.png"),
                            dpi=150, bbox_inches="tight")
                plt.close()
            ...
            if spectral_analysis:
                ...
                plot_energy_spectra(pred_spectra, truth_spectra, step,
...
def _aggregate_multi_ic_metrics(all_step_metrics, all_ml_times, all_solver_times):
    ...
    for key in all_step_metrics[0].keys():
        ...
        summary[f"{key}_mean"] = float(np.mean(all_values)) if all_values else float("nan")
        summary[f"{key}_std"] = float(np.std(all_values)) if all_values else float("nan")
    ...
    summary["speedup_mean"] = (
        ...
        if summary["ml_time_mean"] > 0 else float("nan")
    )
...
def main():
    ...
    parser.add_argument("--spectral_analysis", action="store_true", default=True,
    ...
    print("-" * 70)
    print(f"{'ML time (s)':12s}: {results['ml_time_mean']:.3f} ± {results['ml_time_std']:.3f}")
    print(f"{'Solver (s)':12s}: {results['solver_time_mean']:.3f} ± {results['solver_time_std']:.3f}")
