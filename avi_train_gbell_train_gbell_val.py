...
class GaussianBellsPhiWrapperWithWinds(torch.utils.data.Dataset):
    """
    Wrap PdeDatasetWithWinds so that:
      - inp_fields[0] (geopotential/height channel) = Gaussian bells (scaled like original)
      - winds + target are left as the base dataset provides
    Output stays: (inp_fields, inp_winds, tar_fields, tar_winds)
    """
    ...
    def __init__(
        ...
        # IMPORTANT: assumes PdeDatasetWithWinds has .solver like PdeDataset
        self.solver = base_dataset.solver
        ...
        with torch.inference_mode():
            ref_spec = self.solver.random_initial_condition(mach=0.2)
    ...
    def __getitem__(self, idx):
        inp_fields, inp_winds, tar_fields, tar_winds = self.base[idx]

        phi_grid = self._sample_phi_bells_grid(idx).to(inp_fields.dtype)

        inp_fields = inp_fields.clone()
        inp_fields[0] = phi_grid
